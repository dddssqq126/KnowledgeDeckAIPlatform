...
async def _judge_coverage(
    *,
    ...
    s = get_settings()
    safe_user_message = _trim_text_window(
        user_message, max_chars=s.rag_coverage_user_message_max_chars
    )
    safe_retrieval_query = _trim_text_window(
        retrieval_query, max_chars=s.rag_coverage_query_max_chars
    )
    safe_context = _trim_text_window(
        context, max_chars=s.rag_coverage_context_max_chars
    )
    prompt = (
        f"User question:\n{safe_user_message}\n\n"
        f"Retrieval query:\n{safe_retrieval_query}\n\n"
        "Retrieved context:\n"
        f"{safe_context if safe_context else '[no retrieved context]'}"
    )
    try:
