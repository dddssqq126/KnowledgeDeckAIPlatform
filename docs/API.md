...
| `invalid_extension` | Upload file extension not in `{txt, pdf, cs, md, docx, pptx, xlsx, csv, py, html, css}` |
...
**Request**: `multipart/form-data` with field `file`. Accepted extensions: `txt`, `md`, `pdf`, `cs`, `py`, `html`, `css`, `docx`, `pptx`, `xlsx`, `csv`. 50 MB cap.
